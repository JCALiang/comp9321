...
def login():
    r = request.get_json()
    if r is None:
        return jsonify(Error="Require parameters: username and password"), 404
...
def get_lga(item):
    ...
    if request.args.get('tag') == '0':
        ...
        entry = [FeedEntry(id="http://127.0.0.1:5000/lga/" + item, content_type="xhtml", content=xml,
                           author="Chieh An Liang",
    ...
    for i in range(len(params)):
        ...
        if params[i] in ['and', 'or']:
            ...
            if separator == 'or':
                ...
                xml = qr.decode('utf-8')
                xml = xml[63:-7]
                feed.add(FeedEntry(id="http://127.0.0.1:5000/lga/" + lgaValue1, content_type="xhtml", content=xml,
                                   author="Chieh An Liang",
